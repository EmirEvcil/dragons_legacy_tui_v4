...
from dragons_legacy.models.bag import BagSlot, BAG_SLOT_MAX, BAG_SLOT_COUNT
...
async def _ensure_bag_slots(character_id: int, db: AsyncSession):
    """Ensure a character has exactly 2 bag slot rows."""
    result = await db.execute(
...
async def add_to_bag(
    ...
    if not inv_item or inv_item.quantity < quantity:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Not enough in inventory. Have {available}, need {quantity}."
        )
...
class FightInstance:
    ...
    def process_mob_attack(self) -> dict:
        """Process a mob attack on the player."""
        if not self.is_active or self.turn != "mob":
            return {"error": "Not mob's turn"}

        result = {"attacker": "mob", "events": []}

        base_damage = self._calc_mob_base_damage()

        # Check player evasion (dodge)
        evasion_roll = random.randint(1, 100)
        if evasion_roll <= self.player["evasion"]:
            result["damage"] = 0
            result["events"].append("dodge")
            result["log"] = f"{self.player['nickname']} dodged the attack!"
            self.turn = "player"
            return result

        # Check player block
        block_roll = random.randint(1, 100)
        if block_roll <= self.player["block_chance"]:
            blocked_damage = max(1, base_damage // 2)
            # Apply damage reduction
            if self.player["damage_reduction"] > 0:
                blocked_damage = max(1, int(blocked_damage * (1 - self.player["damage_reduction"] / 100)))
            result["damage"] = blocked_damage
            result["events"].append("block")
            self.player["hp"] = max(0, self.player["hp"] - blocked_damage)
            result["log"] = f"{self.player['nickname']} blocked! Reduced damage: {blocked_damage}"
        else:
            # Check mob critical hit
            crit_roll = random.randint(1, 100)
            if crit_roll <= self.mob["crit_chance"]:
                crit_damage = int(base_damage * 1.5)
                # Apply player defense and damage reduction
                effective_defense = self.player["defense"]
                crit_damage = max(1, crit_damage - effective_defense // 3)
                if self.player["damage_reduction"] > 0:
                    crit_damage = max(1, int(crit_damage * (1 - self.player["damage_reduction"] / 100)))
                result["damage"] = crit_damage
                result["events"].append("critical")
                self.player["hp"] = max(0, self.player["hp"] - crit_damage)
                result["log"] = f"CRITICAL HIT! {self.mob['display_name']} deals {crit_damage} damage to {self.player['nickname']}!"
            else:
                # Normal attack with defense mitigation
                effective_defense = self.player["defense"]
                final_damage = max(1, base_damage - effective_defense // 4)
                if self.player["damage_reduction"] > 0:
                    final_damage = max(1, int(final_damage * (1 - self.player["damage_reduction"] / 100)))
                result["damage"] = final_damage
                self.player["hp"] = max(0, self.player["hp"] - final_damage)
                result["log"] = f"{self.mob['display_name']} deals {final_damage} damage to {self.player['nickname']}."

        # Track damage dealt by mob (= damage taken by player)
        dmg = result.get("damage", 0)
