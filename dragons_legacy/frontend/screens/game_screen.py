...
class UsePotionQuantityModal(ModalScreen):
    ...
    def __init__(self, item: dict, max_quantity: int, bag_slots: list[dict]):
        super().__init__()
        self.item = item
    ...
    def compose(self) -> ComposeResult:
        ...
        for s in self.bag_slots:
            ...
            if s_name:
                slot_lines.append(f"Slot {idx + 1}: {s_name} x{s_qty}")
            else:
                slot_lines.append(f"Slot {idx + 1}: Empty")
        ...
        with Container(id="use-qty-dialog"):
            ...
            with Horizontal(classes="use-qty-slot-row"):
                for s in self.bag_slots:
                    ...
                    s_qty = s.get("quantity", 0)
                    if s_name:
                        label = f"Slot {idx + 1}: {s_name} x{s_qty}"
                    else:
        ...
        if event.button.id.startswith("use_qty_slot_"):
            ...
            if slot_data:
                ...
                if s_name and s_name != item_name:
                    self._show_error(f"Slot {slot_idx + 1} contains {s_name}. Empty it first.")
                    return
                current_qty = slot_data.get("quantity", 0) if s_name == item_name else 0
                if current_qty + qty > self.max_quantity:
                    self._show_error(f"Would exceed max. Slot has {current_qty}, max is {self.max_quantity}.")
                    return
...
class GameScreen(Screen):
    ...
    def _open_item_modal(self, item: dict) -> None:
        ...
        is_consumable = (slot == "consumable") and not is_equipped
    ...
    def _use_consumable_item(self, item: dict) -> None:
        ...
        from dragons_legacy.models.bag import BAG_SLOT_MAX
        ...
        async def _load_and_show():
            ...
            self.app.push_screen(
                UsePotionQuantityModal(item, max_qty, bag_slots),
    ...
    async def _show_bag(self) -> None:
        ...
        for s in bag_slots:
            ...
            s_qty = s.get("quantity", 0)
            if s_name:
                lines.append(f"  Slot {idx + 1}: {s_name} x{s_qty}")
            else:
        ...
        for s in bag_slots:
            ...
            s_qty = s.get("quantity", 0)
            if s_name:
                label = f"🧪 Slot {idx + 1}: {s_name} x{s_qty} — [Remove]"
