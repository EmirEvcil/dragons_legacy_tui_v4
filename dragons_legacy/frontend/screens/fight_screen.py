...
class FightScreen(Screen):
            ...
            with Horizontal(classes="fight-main"):
                ...
                with Vertical(classes="fight-right-panel"):
                    ...
                    with Horizontal(classes="fight-participants-panel"):
                        # Player side
                        with Vertical(classes="fight-player-side"):
        ...
        while True:
            ...
            if msg_type == "fight_started":
                ...
                if message["state"].get("turn") == "player":
                    ...
                    self._update_turn_label("Mob's turn...")
    ...
    def _set_player_turn(self, is_player: bool) -> None:
        ...
        if is_player:
            ...
            self._update_turn_label("Waiting...")
    ...
    def _update_bag_slot_buttons(self) -> None:
        ...
        for idx in range(2):
            ...
            if slot and slot.get("item_name") and slot.get("quantity", 0) > 0:
                ...
                base_label = f"Slot {idx + 1}: {item_name} x{slot['quantity']}"
                ...
                if cd > 0:
                    ...
                    btn.label = f"Slot {idx + 1}: {item_name} x{slot['quantity']} (Used)"
                    should_disable = True
                else:
                    btn.label = base_label
                    should_disable = not self._fight_active
            else:
                btn.label = f"Slot {idx + 1}: Empty"
                should_disable = True
